...
export function AdminOrderAlerts({token}:{token:string}) {
 ...
 return <aside className="admin-order-alerts" aria-label="Avisos de pedidos"><div role="status" aria-live="polite">{failed?'No pudimos actualizar los avisos. Revisá la conexión y la sesión.':pending===null?'Consultando pedidos…':`${pending} pedidos pendientes`}{fresh>0&&` · ${fresh} nuevos desde que abriste el panel`}</div><Link to="/admin/pedidos" onClick={()=>setFresh(0)}>Ver pedidos</Link><button type="button" onClick={()=>void toggleSound()}>{sound?'Silenciar aviso':'Activar aviso sonoro'}</button><small>Actualiza cada 20 s con el panel abierto. El navegador puede pausar los avisos en segundo plano.{audioError?' No se pudo activar el audio en este navegador.':''}</small></aside>;
}
